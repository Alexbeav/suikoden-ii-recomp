...
#
# Writes: dist/suikodenii-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix suikodenii \
  --exe-name Suikoden_II \
  --omit-openbios \
  --display-name "Suikoden II" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Suikoden II (USA) disc" \
  --runtime-dir mods \
